  ...
  if (cmd === 'offset') {
    ...
    if (cmd === 'fleet') {
      ...
      const repairLines = repairActive.length
        ? repairActive.map(t => `第${t.slot}ドック: ${t.name || '修理中'} ${formatRemain(t.endAt)}`)
        : ['全ドック空き'];
      ...
      const buildLines = buildActive.length
        ? buildActive.map(t => `第${t.slot}ドック: ${t.name || '建造中'} ${formatRemain(t.endAt)}`)
        : ['全建造ドック空き'];
      ...
      for (const t of state.timers) {
        ...
        if (t.kind === 'expedition' && Number.isSafeInteger(t.endAt)) {
          ...
          const events = (t.events || []).filter(e => Number.isSafeInteger(e.endAt) && e.endAt > now - 60000);
